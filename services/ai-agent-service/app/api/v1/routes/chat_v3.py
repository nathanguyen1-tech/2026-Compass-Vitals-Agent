"""Chat V3 API endpoint — POST /api/v1/chat-v3.

Connects patient messages to Intake Agent V3 (two-LLM per turn architecture).
...
from app.agents.intake_agent_v3 import intake_node_v3, _initial_greeting_v3
...
async def chat_v3(request: ChatRequest):
    ...
    session = _sessions_v3.get(session_id)
    ...
    if session is None:
        ...
        _sessions_v3[session_id] = session
    ...
    if is_new_session and (not request.message or not request.message.strip()):
        greeting_result = _initial_greeting_v3(session)
